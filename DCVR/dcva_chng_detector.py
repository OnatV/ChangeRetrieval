...
net_loc       = 'trainedNet/RGB/trainedModelFinal'
...
inputChannels = 'RGB'
outputLayerNumbers = np.array([2,8,23])
thresholdingStrategy = 'scaledOtsu'
otsuScalingFactor = 0.75
objectMinSize = 128
...
def change_detector(preChangeImage,postChangeImage):
    ...
    if thresholdingStrategy == 'scaledOtsu':
    ...
    # result = np.repeat(np.expand_dims(cdMap,2),3,2).astype(float)
    
    # plt.imshow(result)

    result = cdMap.astype(int)
